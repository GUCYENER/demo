...
- [x] **T1 (METIS+ORACLE)** — `metric_ambiguity.py`: detect + heuristik enum (ranking-regex + agg_func=None + build_metric_schema_context reuse). Unit ★★★.
- [x] **T2 (METIS)** — `metric_ambiguity_gate_node` + `metric_clarification_node` (interrupt/resume, chosen_metric map).
- [x] **T3 (HERMES+METIS)** — `graph.py` 2 node + route_after_metric_ambiguity (table-clarify sonrası).
- [x] **T4 (ORACLE)** — `sql_generate.py` chosen_metric inject (additive) + [→EVAL] prompt.
- [x] **T5 (HERMES)** — `agentic_query_api.py` SSE kind passthrough + /resume metrik-resume.
- [x] **T6 (ATHENA+HEBE)** — FE metrik-kart varyantı + HEBE a11y/marka gate.
- [x] **T7 (TYCHE)** — ~14 unit + standalone runner + gstack E2E (metrik-belirsiz soru → kart → seç → SQL).
...
## İmplementasyon Kaydı — Dilim-2b: deep_think PORT (2026-06-07, LIVE)
**GROUNDING DÜZELTMESİ (varsayım yapma):** FE grounding'inde agentic path'in (engine'in olduğu yer)
**canlı FE tüketicisi OLMADIĞI** çıktı (`AgenticQueryConsumer.run` caller yok; ana chat deep_think).
Kullanıcı kararı: **deep_think'e PORT** (kullanıcının gerçek serbest-metin chat'i). Agentic engine
(commit 1fe92c1) forward-infra olarak flag-off kalır.

- **PORT (additive):** `metric_ambiguity.py` detektör şekil-esnek (name/table_name); `deep_think_service.
  process_stream_db_only` 4c.5 metrik-gate (table çözülünce, metric_hint yoksa detect → yield
  clarification kind:metric + return; metric_hint varsa extra_context'e ölçü ekle); metric_hint zinciri
  `dialog.py → processor.py → process_stream_db_only`; FE `dialog_chat_utils.renderMetricClarifyCard`
  (db-disambig-* reuse, HEBE: role/aria/escape/type) + `dialog_chat.js` clarification handler kind:metric
  branch + `_sendDbMessageWithHint` metricHint param + showToast. **deep_think'te LIVE (flag yok).**
- **🛡️ ADVERSARIAL (2 ajan) → 2 GERÇEK REGRESYON yakalandı→düzeltildi:** (F1) metrik-gate golden-SQL
  (≥0.95 doğrulanmış)/cache kısa-devresini (2549/2558) önlüyordu → gate'e `not golden_hit and not cached
  and not follow_up_context` guard. (F2) follow-up'ta tetikleyip re-send'de takip-çıpasını kaybediyordu →
  aynı guard. + 2 hardening: FE aria-label `&quot;` escape, detektör "ilk N" (TR top-N). FE: tablo-clarify
  REGRESYON YOK (branch additive, return'lü), XSS güvenli, contract uçtan-uca tutarlı.
- **Doğrulama:** py_compile 5/5 · **20/20 unit** (deep_think-şekil + ilk-N dahil) · FE bundle rebuild · 2 adversarial ajan (fix sonrası temiz).
- **BEKLEYEN:** canlı render/e2e (kullanıcı live-test) — metrik-belirsiz soru → kart → seç → doğru SQL. Backend restart + bundle (`?v=` cache-bust). Migration YOK. v3.79.0.
