...
Last updated: 2026-05-25 (R019/R020 re-target v3.35.0 → v3.37.0 — v3.35/v3.36 sprintleri farkli scope kapatti, FE refactor v3.37 sprintine ertelendi)
...
| R019 | P2       | single-file | low  | M      | v3.37.0 | 2026-05-24 | dialog_chat.js — 4 raw fetch call-site migration to new vyraFetch SSE/blob variants (follow-up to R016) | frontend/assets/js/modules/dialog_chat.js | open | R016 api-side ready; refactor SSE×2 (lines 551, 1433) → `responseType:'stream'` + AbortController; blob×2 (lines 2656, 2988) → `responseType:'blob'`. Defer to a focused PR to keep diff reviewable; needs SSE happy-path manual smoke (send-message round-trip). **Re-target 2026-05-25:** v3.35.0 → v3.37.0 (v3.35/v3.36 sprintleri farkli scope kapatti; ZEUS karari, kullanici onayli) |
| R020 | P2       | repo-wide   | low  | M      | v3.37.0 | 2026-05-24 | vyraFetchUI adoption across 28 modules (follow-up to R017) | frontend/assets/js/modules/* (28 files) | open | R017 wrapper ready. Adoption strategy: migrate per-module in batches of 4-5; assert behaviour parity (toast text, loading-state) per module. Boilerplate save ~50-60% per call-site; no functional change expected. **Re-target 2026-05-25:** v3.35.0 → v3.37.0 (R019 ile beraber FE refactor sprintinde) |
| R021 | P3       | repo-wide   | low  | XL     | v3.40.0 | 2026-05-28 | Smart Discovery 2026-05-24 audit P2/P3 backlog overflow (~40 finding) | (audit silindi, git history) | open | Audit dosyası `.agents/audits/2026-05-24_smart_discovery_findings.md` 2026-05-28 sprint temizliğinde silindi. Audit P0 (4 finding) ve P1 (21 finding) v3.34.3-v3.37.x sprintlerinde absorbe edildi (mevcut R005-R020 done satırları + commit log'lar). P2 (26) + P3 (14) = ~40 finding orijinal audit'ten manuel triage edilmedi. Yeniden açmak için: `git log --diff-filter=D --follow -- .agents/audits/2026-05-24_smart_discovery_findings.md` ile son içerik bulunup mod 4 explore (frontend/backend/llm/sec-perf) per dosya re-triage gerekir. Defer reason: audit eski, scope drift riski yüksek — yeniden yapılırsa fresh audit önerilir |
| R022 | P1       | single-file | medium | M    | v3.50.0 | 2026-06-02 | `tests/db_smart/test_api_db_smart.py` 8 PRE-EXISTING failure (branch hira HEAD'de fail) | tests/db_smart/test_api_db_smart.py | open | v3.49.0 sırasında tespit. Fail edenler: `test_list_columns_envelope`, `test_explain_endpoint_{rejects_non_select,renders_and_caches,invalid_ast_render_400}`, `test_list_saved_reports`, `test_mark_run_success`, `test_execute_stream_{returns_sse_response,rejects_unauthorized_table}`. **Stash ile kanıtlandı:** commit a23648e (değişiklik YOK) ile de 8 fail / 64 passed → bu işle İLGİSİZ. İki sınıf: (a) mesaj-wording drift (execute_stream_rejects "yetkiniz" bekliyor, mesaj "yetkili olmadığınız" — v3.39.2 _scope_restricted_message testi güncellemedi); (b) TypeError (list_saved_reports/mark_run — muhtemel mock/signature drift). Aksiyon: her failure tek tek → kod regresyonu mu stale test mi ayır; kod doğruysa testi senkronla. |
