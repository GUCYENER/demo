...
| R022 | P1       | single-file | medium | M    | v3.51.0 | 2026-06-02 | `tests/db_smart/test_api_db_smart.py` 8 PRE-EXISTING failure (branch hira HEAD'de fail) | tests/db_smart/test_api_db_smart.py | done       | v3.51.0'da kapatıldı (72/72 passed). 7 stale test güncel doğru davranışa hizalandı (F17 graceful explain 200, v3.38 scope gate 404, v3.40 enforce_sql_scope, v3.41.5 name_exact, v3.38.7 snapshot, v3.39.2 _scope_restricted mesaj). 1 GERÇEK kod bug'ı düzeltildi: post_explain_ast _select_empty yalnız ast.get("select") bakıyordu → kanonik "columns"-keyed AST (ast_renderer dual-key) has_ast:false'a düşüp EXPLAIN/cost hiç hesaplanmıyordu → columns\|\|select dual-key. Plan: .agents/plans/2026-06-02_1200_r022_stale_tests_v1.md |
