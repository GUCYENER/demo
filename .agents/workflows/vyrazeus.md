...
6. **Plan Tarama (HERA):**
   - `.agents/plans/` klasöründe `status: in_progress` olan plan varsa yüklenir, `last_commit` ile git'in mevcut HEAD'i karşılaştırılır
   - Sapma varsa kullanıcıya bildirilir (plan stale)
...
### Dosya Adı Kuralı
- `.agents/plans/{vYYY.Z}_{kisa-slug}.md` (örn `v3.29.7_faz6_eksik_implementasyon.md`)
- Bug fix → `.agents/plans/{vYYY.Z}_fix_{slug}.md`
