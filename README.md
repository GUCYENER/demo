...
### 🆕 v3.43.0 (2026-06-01) - DB Keşif Sertleştirme (HEPHAESTUS + HERMES + ATHENA + ARES + TYCHE)
> **4 faz + canlı düzeltmeler.** (P0-A) Re-keşif admin onaylarını artık silmiyor — `detect_objects` koşulsuz enrichment DELETE'i kaldırıldı, diff-tabanlı `_invalidate_enrichments_on_diff` (silinen→arşiv, değişen→`schema_hash=NULL` re-enrich admin alanları korunarak, geri-eklenen→reaktivasyon, kaldırılan kolon→orphan temizliği). (P0-B) `collect_samples` boyut-farkında RASTGELE örnekleme (4 dialect TABLESAMPLE/SAMPLE/ORDER BY random) + per-statement timeout + rollback; `randomize=False` geri-uyumlu. (P1-C) `enrich_tables_batch` `ThreadPoolExecutor(max_workers=4)` — worker başına `get_db_conn` (~4× hız). (P1-D) `ds_discovery_jobs.progress_*` (mig 050 + SCHEMA_SQL garanti) + `update_job_progress` + frontend "X/N tablo". (P2-E) `create_job` advisory-lock TOCTOU + `_auto_invalidate` content_type-agnostic orphan.
> **Canlı kök fixler:** `get_pending_approvals`/`get_approved_enrichments` RealDictCursor `dict(zip)` çöp→dual-mode (admin paneli kolon adı gösteriyordu); `system_logs.request_id` UndefinedColumn crash → SCHEMA_SQL index ÖNCESİ `ADD COLUMN IF NOT EXISTS`; LLM JSON parse → `_coerce_llm_json` çok-stratejili (extract_json_obj + quote-aware trailing-comma + literal-newline) + prompt sertleştirme; `get_active_llm` `finally` connection iade.
> **BİTİR `/code-review medium`:** HIGH bulgu (trailing-comma regex string içeriğini bozuyordu) → quote-aware walker ile giderildi + smoke doğrulama.

### 🆕 v3.41.1 (2026-05-31) - "Veritabanında Ara" perf + dialect fix (NIKE + POSEIDON + METIS + ORACLE)
> **Sorun (kullanıcı):** "fatura listesi ver" sorgusu 285s sürdü, max 15dk; "çalıştırma sürecinde sorun var." **Kanıtlı kök (varsayım DEĞİL — log + manuel test):** embedding/LLM değil (ONNX 3.6s, Azure gpt-4.1-mini hızlı); **Oracle bağlantısı erişilemez listener'a connect ederken timeout'suz ~430s asılıyor** (manuel test ELAPSED=430.7s; sonra ORA-12170 ile 32.5s'e indi) ve **self-heal bunu LLM-regenerate+reconnect döngüsüne sokuyor**.
> **Fixler:** (1) `ds_learning_service._get_db_connector` Oracle connect'ine **DSN descriptor CONNECT_TIMEOUT=15** (+TRANSPORT_CONNECT_TIMEOUT/RETRY_COUNT; PG/MySQL/MSSQL'de connect_timeout=15 vardı, Oracle'da YOKtu) → erişilemezse ~15-32s hızlı fail, 430s değil. Kanıt: ORA-12170 @15s, thick mode. (2) `deep_think_service` self-heal **bağlantı/altyapı hatasında (ORA-28547/12xxx/TNS) ATLANIR** (`_is_infra_db_error` guard) — SQL doğru, sorun bağlantı; LLM-regenerate boşuna + döngü yaratıyordu → fail-fast. (3) `post_preview` base SQL dialect'i **kaynaktan** çözer (FE `_buildWizardState` hardcoded 'postgresql' → Oracle kaynakta base SQL `LIMIT` yanlıştı; "Seçim vs Nihai" modalı tutarsızdı) → Oracle'da `FETCH FIRST`.
> **NOT (ortam):** Oracle listener `localhost:1521`/`FREEPDB1` app'ten erişilemiyor (ORA-12170 connect timeout, "açık" olsa bile) — listener binding/port/PDB servis kaydı/Docker network kullanıcı tarafında çözülmeli. Bu fixler hang'i **sınırlar + net hataya çevirir**, Oracle'ı erişilebilir YAPMAZ.
...
### 🆕 v3.39.1 (2026-05-31) - Wizard SQL önizleme alias KÖK fix + eksik i18n key'ler (ORACLE + HEBE + APOLLO + TYCHE)
> **Alias KÖK fix:** Önizleme/Çalıştır öncesi SQL üretimi, kolon ÇIKTI alias'ı boşluk/Türkçe içerdiğinde ("Adres Kimliği") `assembly failed: Invalid identifier: 'Adres Kimliği'` veriyordu — `ast_renderer._render_column` alias'ı bare-identifier sanıp `_validate_ident` ile reddediyordu. Çıktı alias'ı bir REFERANS değil, sonuç başlığıdır → yeni `_quote_output_alias` ile güvenli tırnaklanır (kapatma-tırnağı ikilenir = injection guard; kontrol baytı reddi; 128 char cap). 4 dialect doğrulandı (`"ADRES_ID" AS "Adres Kimliği"` / `[...]` / `` `...` ``); `expr` (fiziksel kolon) strict kalır. 104 ast_renderer testi + 4 yeni regresyon yeşil. (Bug pre-existing'di; v3.39.0 SQL önizleme butonu görünür yaptı — kullanıcı raporu.)
> **i18n:** Wizard'da ham key gösteren 8 eksik anahtar TR+EN eklendi: `toast.select_columns_first`, `toast.format_select_columns_first`, `toast.format_select_metric_first`, `toast.report_saved`, `error.permission_denied/auth_expired/not_found`, `banner.llm_fallback_subtitle`. (i18n JSON runtime-fetch — bundle rebuild gerekmez, hard-refresh.)

### 🆕 v3.39.0 (2026-05-30) - Tablo-yetki KÖK fix + Wizard SQL önizleme + Graphify manuel (HEPHAESTUS + ARES + HERMES + ATHENA + HEBE + TYCHE + MNEMOSYNE-GRAPH)
> **Güvenlik (KÖK):** v3.38.0 tablo-bazlı yetkilendirme **herkes için bozuktu** — `data_source_access.user_accessible_tables` RealDictCursor satırlarını tuple-unpack edip kolon ADLARINI okuyordu → scope hep çöp (`('schema_name','table_name')`); restricted kullanıcı hiçbir şey göremiyor, admin her şeyi bypass ediyordu. Ampirik kanıt: fix sonrası gerçek 3 tabloyu döndürüyor. Dict|tuple uyumlu okuma. **Admin (kullanıcı kararı / Opsiyon A):** admin artık otomatik bypass DEĞİL — kaynakta açık `restricted` grant varsa kısıt admin'e de uygulanır (managed-admin, `_subject_has_any_grant`); grant yoksa tüm tablolar. **Saved-report rerun gate:** `post_execute_stream` `allowed_tables=None` geçip SafeSQLExecutor whitelist'ini ATLIYORDU → `resolve_scope(can_execute)` + `check_table_whitelist`; yetkisiz tablo içeren rerun → 403 Türkçe ("yetkiniz yok"), sonuç gösterilmez. Picker (`search_tables`) ek koda gerek kalmadan düzeldi. 27 test yeşil, 0 yeni regresyon.
> **Wizard SQL önizleme:** Önizleme footer'ında en sol "📄 SQL" butonu — çalıştırma öncesi SQL'i pretty-print + talep (`user_intent`) ile `VyraModal`'da gösterir, kopyalanabilir (kullanıcı "ne istedim / ne SQL / mantık doğru mu" review eder). XSS: `_escape`; HEBE gate uygulandı.
> **Graphify manuel:** `graphify.bat` (sistem Python313 + `USE_TF=0` → embedding `embed_errors: 91→0`; status→init→mine→wakeup), `start.ps1`'den `[0/7]` Graphify bloğu çıkarıldı, `mcp_warmup.bat` silindi, `.mcp.json` env `USE_TF=0` (local, gitignore).
> **Backlog (RB-v3.39.0):** schedule_runner/query_builder/query_state execute yolları aynı tablo-yetki gate'i alıyor mu (audit) + `check_table_whitelist` şema-agnostik sınırlaması (çok-şemalı kaynakta aynı-adlı tablo) notu.
...
- 🔗 **RC3 — v3.29.8 entegrasyon:** `multi_signal_rank.build_centrality_index` artık confidence-weighted: declared/verified=1.0, inferred unverified=0.5×conf, rejected=0. `signal_weight_analyzer` yeni `min_event_age_hours` parametresi — `fk_centrality` örneklemi Pearson öncesi `event_created_at >= deploy_ts + N hours` filtreli (deploy-day spike izolasyonu); diğer sinyaller etkilenmez. `signal_weight_api /current` yanıtına `fk_inference_recent_deploy`/`age_hours`/`deploy_ts` eklendi. `text_to_sql.get_schema_context` FK SELECT: `WHERE rejected_at IS NULL AND (is_inferred=FALSE OR admin_verified=TRUE OR confidence_score >= 0.70)`. 16 entegrasyon testi.
- 🖥️ **RC4 — UI:** `signal_weight_tuner.js` deploy banner (72s pencere, kalan saat/gün gösterimi). `agentic_observability` 5. tab "FK Inference" — kaynak seçici + 7 stat card (toplam/declared/inferred/verified/pending/rejected/avg conf) + pending tablo. `ds_learning_module` ilişkiler tablosuna `Tür` kolonu (Declared/Inferred/Inferred ✓) + "FK Çıkarımını Yenile" butonu.
- 🛡️ **Defense-in-depth:** Identifier whitelist regex, parametrize sample validation SQL, per-dialect statement_timeout (PG `SET LOCAL statement_timeout`, MSSQL `LOCK_TIMEOUT`, MySQL `MAX_EXECUTION_TIME` hint, Oracle resource limit fallback), `rejected_at IS NULL` her filter'da, low-confidence inferred FK schema_context'ten hariç (≥0.70 eşik).
- 🧪 **Test:** 89 yeşil (24 dialect + 26 service + 16 integration + 12 analyzer regresyon + 11 cache regresyon).
...
- 🧹 **14 yeni tablo:** `app/api/routes/system.py` reset rutinine eklendi — `learned_db_queries`, `ds_synthetic_query_runs`, `ds_column_embeddings`, `agentic_query_decisions`, `agentic_query_feedback`, `agentic_size_observations`, `catboost_models`, `few_shot_examples`, `business_glossary`, `metric_definitions`, `synonym_suggestions`, `pipeline_events`, `pipeline_traces`, `user_preferences`. Her biri `information_schema.tables` lookup guard ile sarıldı — eski deploy'larda tablo yoksa loop crash etmez, sadece `deleted_counts[tbl] = 0` döner. `company_id` filtresi varsa onunla, yoksa global delete. `ds_column_embeddings` sadece `source_id` üzerinden filtrelenir.
- 📊 **`/system/info` count:** Aynı 14 tablo için pre-delete row count loop'u — admin temiz DB discovery testinden önce ne silineceğini görebilir.
- 🎨 **Frontend chip:** `frontend/assets/js/system_manager.js` — `agenticLearningTotal` özetle `fa-brain` ikonlu chip + `data-tooltip` 14 tablo adını listeler. Tehlike modal mesajı agentic öğrenme verisinin silineceğini açıkça belirtir.

### v3.28.3 (2026-05-18) - Faz 5 G4: Pre-execute Drag-Drop Query Builder (HEBE gate)
- 🧩 **Query State AST → SQL:** `app/services/pipeline/nodes/query_state_builder.py` — `build_sql_from_state(state)` drag-drop UI state'inden parametrize SELECT SQL üretir. `_is_safe_identifier()` regex whitelisting (`^[A-Za-z_][\w]{0,62}$`) + `_normalize_op()` allow-list (`=, !=, <, <=, >, >=, LIKE, ILIKE, IS NULL, IS NOT NULL, IN`) + `_clamp_limit()` (1-10000). Dialect-aware quoting: PG/Oracle `"x"`, MySQL `` `x` ``, MSSQL `[x]`; LIMIT vs `FETCH FIRST n ROWS ONLY` vs `TOP n`.
- 🌐 **Backend endpoint:** `POST /api/query-state/preview` — JWT zorunlu, Pydantic v2 model (alias=schema; protected_namespaces=() ile `schema` shadowing warning kapatıldı), `_resolve_dialect(source_id)` DB'den `db_type` okur (graceful fallback `postgresql`). Sample data fetch ETMEZ — sadece sanity SQL döner (frontend editöre yapıştırır). Body: `{source_id?, schema?, table, dialect?, selected_columns[], filters[], order_by?, limit?}`. Response: `{valid, sql, params, dialect, warnings[]}`.
- 🎨 **Frontend modülü:** `frontend/assets/js/modules/query_builder.js` — `window.QueryBuilder.open({parentEl, sourceId, schema, table, columns, dialect, onSqlReady})`. Mevcut kolonlar paneli (chip'ler) → SEÇİLDİ paneli (drag-drop, sıra önemli) → Filtre satırları (kolon + op + value) → Order by + limit → "SQL Önizle" butonu. Backend yanıtı `onSqlReady(sql, params, warnings)` callback'ine geçer.
- ♿ **A11y — tam keyboard alternatifi:** `role="application"` ana kap, alt list'lerde `role="list"` + `role="listitem"`. **Space** chip'i tutar/bırakır, **Enter** sona ekler, **ArrowUp/Down** seçili kolonu yukarı/aşağı taşır, **Escape** tutmayı iptal. `aria-grabbed` (legacy) + `aria-pressed` + `aria-live="polite"` status mesajı. `data-tooltip` yardım metni her chip'te.
- 💅 **CSS modülü:** `frontend/assets/css/modules/query_builder.css` — brand-token kullanan paneller, `prefers-reduced-motion` fallback, dragover highlight, focus-visible outline.
- 🛡️ **Güvenlik:** Tüm identifier'lar regex whitelist'ten geçer; aksi halde drop edilir (warnings'a eklenir). Filtre değerleri **psycopg2/parametrize** ile gönderilir (string concat YOK). Max 50 filtre.
- 🧪 **27 unit test (tests/test_query_state_builder.py):** `TestSafeIdentifier` (valid/invalid/too-long), `TestNormalizeOp` (basic/reject), `TestClampLimit` (default/range/garbage), `TestBuildSQL` (17 — simple select, selected cols, filters eq/in/is-null, order_by, limit clamp, dialect PG/MySQL/MSSQL/Oracle, invalid schema/table/column/op rejected, too-many-filters, non-dict state).

### v3.28.2 (2026-05-18) - Faz 5 G3: Sample Data Preview kartı (HEBE gate)
- 🔭 **Pre-execute önizleme:** SQL üretildikten ama execute edilmeden önce, kullanıcıya aday tablonun cached örnek satırları gösterilir. `app/services/pipeline/nodes/sample_data_preview.py` — `build_sample_preview(cur, source_id, schema, table, limit)`, `pick_top_table_for_preview(final_state)`, `extract_first_table_from_sql(sql)` (regex tabanlı, dialect-aware: PG/MySQL/MSSQL/Oracle quote stilleri).
- 🌐 **REST endpoint:** `GET /api/data-sources/{source_id}/samples?schema&table&limit=5` — `ds_db_samples` cache'inden okur, `ds_db_objects` join ile kolon metadata + `ds_table_enrichments` join ile Türkçe `business_name_tr` (graceful). Permission: `is_admin OR company_id match`. Cache miss → 404 with hint "Yöneticinizden 'Örnek Veri Topla' job'ını çalıştırmasını isteyin".
- 📡 **SSE event:** `deep_think_service.py` SQL üretim sonrası + `agentic_query_api.py` size_prediction öncesi `selected_table_for_preview` (hint: `{source_id, schema, table}`) yayar. `agentic_query_consumer.js` ve `dialog_chat.js` switch'lerine yeni case eklendi.
- 🎨 **Frontend renderer:** `frontend/assets/js/modules/sample_data_preview.js` — `window.renderSampleDataPreview(parentEl, hint)`; skeleton (`.skel-line`) → fetch → kart (`role="region"`, `aria-labelledby`, `aria-busy`). Empty-state CTA, `data-tooltip` ipucu, XSS-safe cell rendering (`_escapeHtml` + 64-char truncate).
- 💅 **CSS modülü:** `frontend/assets/css/modules/sample_data_preview.css` (yeni) — brand-token (`--accent`, `--surface-hover`) kullanan kart, `@keyframes skelShimmer`, `@media (prefers-reduced-motion)` fallback, light-theme overrides. `build.mjs`'ye eklendi.
- 🛡️ **SaaS / HEBE gate (silent):** role/aria, skel-line, empty-state CTA, data-tooltip, brand tokens, prefers-reduced-motion — tümü checklist'e uygun. Cache hit/miss, schema NULL, schema-qualified, columns_json fallback, enrichment error graceful → hepsi unit test ile garanti.
- 🧪 **24 unit test (tests/test_sample_data_preview.py):** `TestExtractFirstTable` (8 — simple/qualified/quote-stilleri/CTE-join/empty), `TestPickTopTable` (6 — selected vs ranked priority, alternative keys, object_name), `TestBuildSamplePreview` (10 — cache hit/miss, NULL schema, limit clamping 1-50, columns fallback from rows, invalid sample_data, enrichment error graceful).
...
- 💎 **HEBE — v3.19 UI polish kalan maddeler:** `data_sources_module.js` edit modal — `role="dialog"` + `aria-modal="true"` + `aria-labelledby` + click-outside + return-focus + close butonu `aria-label="Kapat"` + `data-tooltip="Kapat (Esc)"` (önceki ESC handler korundu). `feature_permissions_module.js` Kaydet butonu loading state + `window.showToast` (önceki sürümlerde tamamlanmıştı, doğrulandı). `data_sources_module.js` ikon-only butonlara `aria-label` + `data-tooltip` (test, discover, history, enrich, dbloop, edit, delete, perm/shield). `_tooltip.css` build.mjs'de yükleniyor, `feature_permissions.css` card aksan renkleri (`[data-feature="kb|db|llm"]` border-left + ikon rengi).
- 🌊 **POSEIDON — Discovery Comment Audit:** 4 dialect (PG, Oracle, MSSQL, MySQL) için native DB comment okuma 2026-05-18 itibariyle DOĞRULANDI — `ds_learning_service.py` satır 313/336 (`col_description`/`obj_description`), satır 538/561 (`sys.extended_properties` MS_Description), satır 827/841 (`all_col_comments`/`all_tab_comments`), satır 643/659 (`INFORMATION_SCHEMA.*.COLUMN_COMMENT`/`TABLE_COMMENT`). Audit doc'u kapandı.
- 📋 **HERA — v3.28 Faz 5 Kalanı Plan Doc:** `.agents/plans/v3.28_faz5_kalani.md` oluşturuldu. Master plan Faz 5'in v3.26/v3.27 sürecinde yapılmayan 4 spesifik boşluğu: G1 Table Ranker, G2 Synthetic `generate_db_query_pairs`, G3 Sample Data Preview kartı, G4 Pre-execute Drag-Drop Query Builder UI. v3.28.0 (G1+G2) + v3.28.1 (G3+G4 — HEBE Pre-Plan Gate) iki alt-sürüme bölündü. Kullanıcı onayı bekliyor.
- 📋 **Plan Güncellemeleri:** `agentic_sql_copilot_master_plan.md` Faz 5 checkbox'ları durum tablosuyla güncellendi (Column/Filter/Join Predictor + Result Size Predictor + AST Builder ✅; Table Ranker + Synthetic Q + Sample Preview + Pre-execute UI ❌). `v3.19_ui_polish.md` kritik blok tüm maddeleri [x] kapatıldı.
...
**Versiyon:** 3.78.1 (**(1) Kriter case-insensitive — kullanıcı 'ext02d059293' yazsa da veri 'EXT02D059293' ile eşleşsin** [kullanıcı bulgusu; PG case-sensitive `=` → 0 satır, büyük-harfle 1 satır kanıtlı]. İKİ KATMAN: (a) `llm_generate_report` **prompt kuralı** — METİN kolon eşitliğinde `UPPER("kolon")=UPPER('değer')` [`schema_cols_block`'a gate'li → LLM tipi bilir; Türkçe i/İ/ı/I locale-bağımlı dürüst-not]; (b) **DETERMİNİSTİK garanti** `_apply_ci_text_equality` — LLM uymasa da WHERE `"kolon"='literal'`'ı METİN kolonda `UPPER`-sarar [TÜM 4 DİALECT: identifier quote 3 stil `"..."`/`` `...` ``/`[...]`, `UPPER` native, validate'ten önce, fail-soft]. **gstack-review [2 ajan]:** integration/dialect/double-wrap/gate/fail-soft TÜMÜ SAĞLAM; **1 CRITICAL alias-collision** [niteliksiz ad SELECT-alias ile çakışıp `UPPER(sayı)` hatası verebilir] → niteliksiz+alias ATLA, nitelikli `"t"."col"` daima sar; cast/JSON (`::text`,`->>`) kapsam-dışı [kabul, prompt backstop]; injection/ReDoS SÖMÜRÜLEMEZ [25+ girdi]. **18/18 standalone test** [bug + 3 dialect + numeric/JOIN-ON/inequality/already-wrapped/ambiguous/escaped-quote/alias atla]. **(2) "Veritabanında Ara" SQL modalı** [kullanıcı bulgusu]: pretty-print hizasız + scroll yok → `.vyra-sql-modal-code` `white-space:pre`+`overflow-x:auto` [satır-satır HİZALI, soft-wrap reflow yok], `.vyra-sql-modal-body` **`min-height:0`** [flexbox-scroll fix: `flex:1+overflow-y:auto` min-height:0 olmadan içeriğe büyüyüp scroll etmiyordu → uzun SQL artık dikey scroll]; **headless-chrome RENDER KANITLI** [satır-satır + body-clip+scroll]. NOT: `llm_generate_report.py` LF-renormalize [.gitattributes `*.py eol=lf`; HEAD CRLF anomalisi → tek-seferlik flip]. Backend restart (case-fix backend) + frontend bundle (`?v=0453dcdc20`). ÖNCEKİ v3.78.0 (**Akıllı Keşif Step-3 Filtre ekranına "📄 SQL Önizle" butonu** [kullanıcı isteği — Step-4'e gitmeden seçim+talep SQL'ini önceden gör]. Not kutusu ("Bu rapordan ne bekliyorsunuz?") altına buton; tıkla → seçimden baseSql üret (`/sessions/{uid}/preview` deterministik assemble) + mevcut **"Seçim vs Nihai" modalını REUSE** et [üst=baseSql seçim, alt=finalSql LLM+talep `_computeFinalSqlForPreview`]. Talep VARSA iki SQL, YOKSA yalnız baseSql. **Reuse-ağırlıklı** [`_onShowSqlClick`/`_computeFinalSqlForPreview`/`_finalSqlCacheKey` DEĞİŞMEDİ]; yeni: Step-3 buton (`#dswPreviewSqlBtn`, mevcut `dsw-llm-btn-sql` stili) + `_onStep3PreviewSqlClick` handler + `_refreshBaseSqlForPreview` + shared `_postPreviewSql` helper. Yalnız `_state.baseSql` yazar [`lastGeneratedSql`/save akışına DOKUNMAZ]. Bug A fix'i sayesinde Step-3 notu (`userNote`) finalSql'i tutarlı besler. Code-review [2 finder]: **F1** assembly-fail backend yorum-SQL'i (`-- assembly failed…`) önizleme sanma → `/^\s*--/` guard + null [bayat baseSql'e düşme yok] + doğru uyarı mesajı; **F2** `/preview` POST tekrarı → shared `_postPreviewSql` [_loadPreview Step-4 + Step-3 ortak, drift yok]. **+ gstack-review (detaylı, 4 ajan — varsayım yok):** **F3** geri-nav (`n<3`) reset `userNote`'u temizliyor ama `user_intent`'i DEĞİL → boş Step-3 kutusuyla bayat "Talebiniz"+nihai-SQL modalda görünürdü → simetrik temizle (line 234, Bug A "iki kutu aynı alan"); **F4** hand-rolled busy → ortak `_llmSetBusy` [aria-busy a11y + `.dsw-llm-btn[aria-busy]` stili + DRY]; **F5** boş-kolon `SELECT *` önizlemesi → en-az-1-kolon guard [Step3→4 nav gate ile tutarlı] + doğru uyarı mesajı. **Security CLEAN** [note `_escape`, SQL `pre.textContent` → XSS yok; Step-3 aynı endpoint/guard → bypass yok; info-leak pre-existing/amplified değil]. Minor accept: `dswPreviewSqlBtn`/`dswShowSqlBtn` ad-benzerliği [farklı step, yorumla çapraz-referans], comments-in-concat [mevcut desen], re-render-detach [benign]. Hoisting/escape/re-entrancy CLEAN. Build exit 0; semboller doğrulandı. Backend YOK; frontend bundle deploy (`?v=6696bad0e9`) + Ctrl+F5. Render: buton mevcut stili reuse + sticky-footer code-review'da düşük-risk → canlıda görsel teyit önerilir. ÖNCEKİ v3.77.11 (**Bug B KÖK FIX — kayıtlı rapor "Çalıştır" → "Sonuç boş"** [kullanıcı bulgusu, /gstack-investigate + repro-doğrulandı]. KÖK NEDEN: psycopg2 PG **server-side (named) cursor**'da `cur.description` execute'tan SONRA `None`; ancak İLK `fetchmany()`'den sonra dolar. `sql_executor_stream._make_stream_callable` description'ı fetch'ten ÖNCE okuyordu → PG'de `cols=[]` → `columns` SSE event'i HİÇ yayılmıyor → FE `result.columns=[]` → **satır gelse BİLE "Sonuç boş"**. Oracle/MySQL/MSSQL standart cursor'da description execute sonrası hazır → etkilenmiyordu (semptom YALNIZ PG kaynaklı kayıtlı raporda; kullanıcının çalışan raporları Oracle/VYRA_TEST, boş çıkan elysion=PG). KANIT: aynı finalSql (`WHERE Username='EXT02D059293'`) Step-4 Çalıştır'da [SafeSQLExecutor buffered] **2 satır** döndü, kayıtlı-rapor rerun'da [/execute/stream named cursor] **boş**. ÇÖZÜM: `cols`'u İLK `fetchmany`'den SONRA oku+yay (her iki cursor türü için güvenli — standart cursor'da description fetch sonrası da geçerli). + **FE savunma** (`report_detail_modal._renderRunResult`): `result.columns` BOŞ DİZİ `[]` JS'te truthy → eski `||` fallback atlanıyordu; `.length` kontrolü + array-satıra `Sütun N` başlık. **NOT:** save YANLIŞ SQL kaydetmiyor [kullanıcı teorisi çürütüldü] — kaydedilen `last_sql` = çalıştırılan finalSql (`generated_sql` aynı değişken, db_smart_api 2885/2972), doğru; sorun YALNIZ rerun-display. Önizleme "Seçim vs Nihai": üst=baseSql [assemble, alias "t", çıplak kolon], alt=finalSql [LLM+talep, nitelikli] — save finalSql'i [executed] saklar. Code-review [2 finder]: correctness 0 bulgu; regresyon 3 düşük [#1 0-satır→"boş" zaten doğru davranış, #2 fetch-anı-timeout başlık-kaybı PG'de zaten yoktu/error gösterilir, #3 array `Sütun N` ile giderildi]. Repro: named-cursor description-after-fetch + fix columns-emit standalone doğrulandı. Backend restart GEREK; migration YOK. ÖNCEKİ v3.77.10 (**kaydedilen rapor NL-beklenti ("Bu rapordan ne bekliyorsunuz?") save/restore fix** [kullanıcı bulgusu — Bug A; frontend]. KÖK NEDEN: `db_smart_wizard.js` `_buildWizardState` wizard_state'e NL-beklentiyi **HİÇ koymuyordu** → save'de kayboluyor + iki restore yolu (`_hydrateFromSavedReport`/`_loadSavedReport`) geri yüklemiyordu → düzenle'de alan boş. **EK BULGU (code-review):** İKİ kutu aynı soruyu soruyor — **Step-3 Filtre `#dswUserNote→userNote`** (kullanıcının ekranı) + **Step-4 footer `#user-intent→user_intent`** — aralarında sync YOK; ilk fix yalnız user_intent'i kaydetseydi filtre-ekranı yorumu yine kaybolurdu. FIX: save **HER İKİ kutuyu** yakalar (`user_intent || userNote`); iki restore yolu da **İKİ `_state` alanına** yazar → düzenle'de hangi ekranda olursan ol metin görünür + tutarlı. Code-review [1 finder]: çekirdek zincir doğru [4 save yolu `_buildWizardState`'ten geçer, 2 restore yolu, footer display]; Issue-2 (iki-kutu tutarsızlık) bu fix'le kapandı; Issue-1 (ölü userNote-fallback comment) düzeltildi. **gstack-review [kritik-pass, kaynak 17-satır → specialists atlandı]:** SQL/race/LLM-trust/XSS CLEAN; **1 bulgu (P2, conf 7/10) — bayat-not taşması:** restore KOŞULLUYDU (eski raporda anahtar yoksa atla) ama save KOŞULSUZ + load `_resetWizardState` çağırmaz [singleton `_state`, 3335 "geri kalanı koru", 2986 reset yalnız `!reportId`] → save'i aktive ederek pencere açıldı: notlu rapor → aynı oturumda **legacy (fix-öncesi) rapor** açılınca önceki not görünürdü; FIX (kullanıcı onaylı): iki restore bloğu **KOŞULSUZ** (anahtar yoksa '') → save ile simetrik, legacy doğru boşalır, fix-sonrası raporlar birebir aynı. Bundle rebuild (JS syntax OK). **Bug B** (kaydedilen rapor çalıştır→boş) AYRI — kullanıcı test edecek [ipucu: img NİHAİ SQL `Username` küçük-harf, login büyük-harf olabilir]. **+ yeni plan** (ayrı): `2026-06-07_filter_screen_sql_preview` — Step-3'e "SQL Önizle" butonu + user_intent'i NİHAİ SQL'de göster [mevcut "Seçim vs Nihai" modalı reuse, v3.78.0 hedef]. Backend YOK; frontend bundle deploy. ÖNCEKİ v3.77.9 (**Hata İzleme INFO seviyesi** [kullanıcı isteği] — Sistem Parametreleri → Hata İzleme ekranına INFO hem **filtre** ("Yalnız INFO") hem **rozet** (mavi sayım) olarak eklendi. TAMAMEN ADDITIVE [mevcut ERROR/CRITICAL/WARNING/ALL/default ERROR+CRITICAL korundu]: backend `_build_errors_where` izinli-seviyeye `INFO` + `/errors/stats by_level` IN'e `INFO`; frontend `<option>` + `_lvlClass` INFO→`em-lvl-info` [eskiden yanlış turuncu'ya düşüyordu = düzeltme] + `_loadStats` rozeti + CSS mavi `#4da3ff` + bundle rebuild. Code-review [1 finder + enum-completeness]: **SQL-safety CLEAN** [`level=%s` parametreli, IN literal — injection yok], XSS CLEAN [sayı], filter↔stats↔list↔export **ALIGNED** [paylaşılan helper]; kozmetik docstring INFO eklendi. **Perf-not** [blokör değil]: INFO en yüksek-hacim + `system_logs` retention'sız → all-time stats tam-tablo tarar; UI hep `since_hours=24` gönderiyor [bounded] → normal kullanımda sorun yok; infra follow-up = retention+index [backlog]. **+ Bekleyen plan konsolidasyonu** [kullanıcı: "G8 ve diğer bekleyeni plana ekle"]: yeni `2026-06-07_hata_izleme_info_ve_bekleyen_backlog` planı 3 aktif planın bekleyenlerini topladı [G8 FK provenance rozetleri, 1.3/1.4 residual, error-log G3-G6, TEMA-2]; **G9 (PK-hem-FK) v3.77.4'le tamamlandı** olarak işaretlendi + 3.3/4.5 v3.77.8'le kısmen kapandı notu. Backend restart; migration YOK. ÖNCEKİ v3.77.8 (**gstack-review — Akıllı Keşif 3-süreç güvenlik denetimi** [kullanıcı: LLM kolon önerisi + metrik öneri + SQL çalıştırma; "hata olmasın"]. 3 fresh-context adversarial agent her süreci ayrı taradı; KODDAN doğrulanan bulgular düzeltildi. **SQL ÇALIŞTIRMA (`sql_executor_stream.py` + `safe_sql_executor.py`):** **C1** stream path'te **statement_timeout YOKtu** (buffered'da vardı) → runaway sorgu kaynak-DB session'ını süresiz kilitliyor (DoS); fix: PG `SET statement_timeout` / MySQL `MAX_EXECUTION_TIME` / Oracle `call_timeout` (cömert `DBSMART_REPORT_QUERY_TIMEOUT_S`/300s). **C2** `stream_safe_sql` SQL'e LIMIT eklemiyordu (docstring aksini diyordu) → PG-dışı sürücüler tüm sonucu belleğe alıp OOM; fix: `apply_row_limit(max_rows+1)` ['truncated' bayrağı korunur, zaten-LIMIT'liye dokunmaz]. **C3b** `validate_sql` `pg_read_file`/`pg_ls_dir`/`pg_stat_file`/`lo_import`… bloklamıyordu + FROM içermediği için tablo-whitelist'ini ATLIYOR → dar-yetkili kullanıcı superuser-rolünde sunucu dosyası okuyabiliyordu (privilege-escalation); fix: PG dosya/LO fonksiyon denylist'i. **LLM KOLON ÖNERİSİ (`ds_enrichment_service.py`):** **F1** kolon INSERT döngüsünde bir kolon hata verince psycopg2 transaction ABORT olup kalan TÜM kolonlar düşüyordu → tablo "hepsi —" (data-loss, "LLM çöktü" sanılırdı); fix: **kolon-başı SAVEPOINT** [ROLLBACK TO + RELEASE]. **METRİK ÖNERİ:** denetim TEMİZ — LLM `formula`'sı kozmetik, hiç SQL'e girmiyor; her sorgu execute öncesi `validate_sql`+whitelist'ten geçiyor → **LLM→SQL injection YOK**. Minörler ertelendi [custom_metric_parser `OR %s IS NULL` kolon-sızması, boş-{} → 502, MEDIAN/RATIO sessiz-düşme]. YÜZEYE ÇIKAN [pre-existing, blokör değil]: C3a whitelist admin'de fail-open [tasarım]. Test: **19 SQL-hardening** [6 dosya-fonksiyon blok + meşru/UNION/`file_id` bozulmadı + apply_row_limit 4 dialect] + **5 enrichment-SAVEPOINT MagicMock** [col_b patlasa col_a/col_c yazılır = poison yok] + FK 27/27 korundu; compile + ruff 0. **NOT:** bu süreçler FK işinden (v3.77.4-7) bağımsız, ayrı dosyalar. Backend restart; migration YOK. ÖNCEKİ v3.77.7 (**gstack-review adversarial hardening** — extension tier'a **tek-kolon-PK guard** (`len(t.pk_columns)==1`). KÖK NEDEN: `src_is_pk` kolon-bazlıdır → KOMPOZİT-PK üyesi (junction-benzeri) de yanlışlıkla `extension` etiketi + `cardinality '1'/'1'` alıyordu; oysa o kolon from-tarafında **N**'dir (1:1 değil). `cardinality_analyzer` ŞEKİL-FARKINDA (`from_pk==[from_col]`) olduğu için tek-tıkla akışta düzeltiyordu → final tutarlı AMA inference yazımı şekil-kör → **sıra-bağımlı tutarsızlık** (analyze atlanırsa yanlış 1:1 kalır + synthetic junction over-generation). Guard ikisini KOŞULSUZ uyumlu yapar: kompozit-PK FK'leri yine çıkar (full/head tier, cardinality'yi analyzer hesaplar = doğru N:1), sole-PK uzantılar (WFINSTANCEID) ETKİLENMEZ. 3 fresh-context adversarial agent (production-failure + integration data-flow + 2.geçiş) tüm 4 değişikliği BİRLİKTE taradı. **2. geçişte DÜZELTİLEN F3 (monoton-floor regresyonu):** floor, hedefi PK-OLMAYAN naming FK'leri de (`to_col not in to_pk` → `'id'`/`'pk'` fallback'i, ör. PK'sız VIEW) 0.80'de donduruyordu → eskiden yapısal demote bunları auto-join'den (≥0.70) eliyordu. Fix: floor YALNIZ `c_to=='1'` (hedef gerçek PK) iken; aksi halde yapısal demote kazanır → bogus FK 0.2'ye iner. Extension + naming+type (gerçek PK) ETKİLENMEZ. + F6 `target_col` annotation (`Optional[str]`→`Optional[Dict]`, kozmetik). YÜZEYE ÇIKAN [blokör DEĞİL — kullanıcı-onaylı auto tasarımı/pre-existing]: F1 extension 0.85 auto-trust false-positive [REPRODÜKSİYON: sole-PK kolon başka tabloya isim-eşleşirse fabrike 1:1; mevcut naming-FK ile AYNI risk sınıfı; admin-reject + diagnostics + synthetic execution-gate kalkanı; TAM kapatma = extension sample-validation, kullanıcı kararına bırakıldı], S2 nested-connection pool direct-connect fallback [pre-existing; graceful skip + 8+ eşzamanlı cross-source keşif gerektirir]. Mekanik: compile + ruff 0 + **27/27 test** [+kompozit-PK 'extension etiketlenmez' + F3 demote assertion]. Backend restart; migration YOK. ÖNCEKİ v3.77.6 (**keşif TEK-TIKLA TAM** — `detect_objects` artık FK çıkarımından sonra (a) çözülemeyen-FK tanılamasını (`persist_fk_diagnostics`) + (b) cardinality/junction analizini (`analyze_relationships`) **keşif-içi otomatik** koşuyor. KÖK NEDEN: ilk/tam keşif (DB Keşfet) yalnız FK inference koşuyordu → cardinality NULL + tanılama boş kalıyor, kullanıcı "FK Çıkarımını Yenile" / "Kardinalite Analizini Yenile" butonlarına ELLE basmak zorundaydı [kullanıcı: "bu butonları hiç kullanmıyorum"]. Artık keşif tek-tıkla tam → cardinality (1:1/1:N/junction) + sentetik şablon seçimi + join ağırlığı + tanılama **kendiliğinden** gelir; iki buton yalnız manuel re-run/debug aracı olur. GÜVENLİK ("çalışan yapıyı bozma"): **AYRI scoped connection** (`get_db_context_scoped(source_id=)`) → `vyra_conn`'a DOKUNMAZ, herhangi bir hata keşfi/snapshot'ı bozmaz; FK zaten commit'li; her ikisi ayrı try/except; `analyze_relationships` monoton confidence-floor (v3.77.x) ile skoru DÜŞÜRMEZ + idempotent [UPSERT/in-place UPDATE]. Pattern zaten `/infer-fks`'te var. Code-review [2 finder, **0 bulgu**]: `_inf` scope güvenli [infer raise ederse blok atlanır], imzalar uyumlu, exception izolasyonu sağlam [outer `try/except _post_err` tüm `with`'i sarar → FK-failure path tetiklenmez], nested-connection **deadlock-yok** [non-blocking ThreadedConnectionPool distinct conn, maxconn=15, single-flight keşif `check_running_job`], GUC `SET LOCAL` transaction-local sızmaz, re-discovery idempotent. Test: **25/25 regresyon** + compile + imza-doğrulama [detect_objects canlı source-DB gerektirir → orkestrasyon glue compile+imza+review ile doğrulandı]. Backend restart; migration YOK. ÖNCEKİ v3.77.5 (**extension FK 1:1 cardinality persist** + CEO-review etki analizi. Extension (PK-hem-FK) FK'ler TANIMI GEREĞİ 1:1 [iki uç da PK]; inference INSERT'i artık `cardinality_from/to='1'` yazıyor. KÖK NEDEN: eskiden NULL kalıyordu → `cardinality_analyzer` koşmadan sentetik üretim koşarsa `_cardinality_aware_kinds` 1:1'i tespit edemiyor [`cf==""≠"1"`] → `AGGREGATE_COUNT` gibi 1:1'de anlamsız şablon üretiyor [count hep 1] + few-shot'a zayıf örnek sızıyordu. analyzer sonradan aynı `1/1`'i hesaplar [from_col=from PK, to_col=to PK] → çakışma yok; diğer tier'lar NULL [analyzer çözer]; UI artık truthful `1:1` rozeti. **CEO-review etki analizi** [kullanıcı sorusu: nereyi etkiledi / tüm DB / akıllı keşif join / sentetik]: 4 dialect **dialect-agnostik** doğrulandı [`normalize_ident` her iki tarafta case-tutarlı: PG/MSSQL/MySQL lower, Oracle UPPER; type-compat `NUMBER=NUMBER`/`bigint=bigint`→`TYPE_CAT_INT`]; `join_planner`/`fk_graph` **yönsüz edge** → extension join PASS; `text_to_sql` conf≥0.70 PASS [NL→SQL şema-bağlamına girer]; `multi_signal_rank`/`sql_generate` PASS; tek bulgu=bu cardinality. Code-review [2 finder, **0 bulgu**: kolon/placeholder/arg 15/15/13 hizalı, CHAR(1) `'1'` uyumlu, tek INSERT, analyzer idempotent recompute; tüketici-impact SAFE]. Test: **25/25** [+extension cardinality persist MagicMock]. Backend restart; migration YOK. ÖNCEKİ v3.77.4 (**PK-HEM-FK uzantı (table-per-type / 1:1 shared-PK) FK çıkarımı** [canlı ONEDESKPG kanıtı: `WFINSTANCEID bigint PK`∈`T_ICT_DYNAMIC_DATA` = `T_WF_INSTANCE.InstanceId bigint PK`, declared FK YOK]. KÖK NEDEN: `_iter_fk_candidates` is_pk kolonları **KOŞULSUZ eliyordu** → bir tablonun PK'sının AYNI ZAMANDA başka tablonun PK'sına 1:1 uzantı olduğu ilişkiler — ONEDESKPG'nin EN yaygın deseni (her `T_*_DYNAMIC_DATA`→`T_WF_INSTANCE`) — HİÇ çıkarılmıyordu → text-to-SQL ana join'leri göremiyor, sentetik üretim boştu. Kullanıcı bulgusu Grafana canlı SQL `BI.InstanceId = DYN.WFINSTANCEID` cast'siz çalışıyor [bigint=bigint] → "tip uyuşmazlığı" YANLIŞ teşhisti, asıl yara PK-skip. **ÇÖZÜM (toplamsal — non-PK akışı BİREBİR korunur):** (1) PK kaynak koşulsuz elenmiyor; `_pk_targets_own_table` öz-surrogate PK [`InstanceId∈T_WF_INSTANCE`: kök==entity-token → atla, eski davranış] vs uzantı PK [`WFINSTANCEID`: kök≠entity 'data' → çöz] ayrımı; (2) yeni `extension` tier **`SCORE_EXTENSION=0.65`** [iki uç da declared PK + isim ilişkili], tip uyumuyla **0.85** → sorgu (0.70)+sentetik (0.80) gate'lerini geçer; (3) çoklu entity-token eşleşmesinde [`T_WF_INSTANCE` vs `RPT_PROBLEM_RELATED_INSTANCE`] **token-hizalı** tiebreak [trailing-token-concat `'wfinstance'∈{twfinstance,wfinstance,instance}`, düz substring değil → `'order'⊄'workorders'`], yalnız PK kaynakta; (4) `_is_target_pk` → boost YALNIZ iki-ucu-PK uzantıya, sahte yükseltme yok. **+ cardinality_analyzer floor fix:** `analyze_relationships` confidence'ı yapısal formülüyle [0.70] **EZİYORDU** → incremental keşif [adım-5 otomatik] extension (0.85)+naming+type (0.80) FK'leri 0.70'e indirip sentetik gate ALTINA düşürüyordu [latent bug, fix'i boşa çıkarırdı]; artık yapısal analiz EK kanıt = inference skorunu **taban** alır [monoton max, asla düşürmez]. Code-review [4 finder+verify]: F1 cardinality-downgrade FIXED, F3 tiebreak-substring FIXED, entity-token-dup→`_entity_token` helper FIXED; F2 junction-misclass REFUTED [composite-PK-tüm-FK=gerçek junction, 1:1 uzantı tek-kolon-PK], F4 'id'-fallback `_is_target_pk` ile gated, F5 0.85-auto-trust tasarım-kararı [tip-uyumu şart + admin/diagnostics gözetim]. Test: **20/20** [extension çözüm+disambiguation+öz-PK-skip+non-PK-regresyon + table-per-type/junction/surrogate edge + cardinality-floor MagicMock]. Backend restart gerek; migration YOK. ÖNCEKİ v3.77.3 (sentetik SQL üretim eşiği fix [ekip kararı, canlı teşhis]: synthetic generator inferred-FK eşiği **`_MIN_INFERRED_CONFIDENCE` 0.85→0.80**. KÖK NEDEN: sample-validation'sız FK üretim tavanı `SCORE_NAMING(0.60)+SCORE_TYPE(0.20)=0.80` → ONEDESKPG gibi **declared-FK'siz** kaynakta TÜM inferred FK 0.80'de sıkışıp 0.85 kapısına takılıyordu → "Sentetik SQL Üret (FK Loop)" **HİÇ** üretmiyordu (canlı kanıt: 4593 FK → conf≥0.85=0, 2535 tanesi tam 0.80 → synthetic-uygun yalnız 29 declared, onlar da admin_approved enrichment kapısında elenince s3=0). GÜVENLİK: generator zaten ürettiği her SQL'i hedef DB'de **çalıştırıp `row_count>0` döneni öğreniyor** → execution = veri-doğrulama, ön-eşik gereksiz koruma; zayıf head/fuzzy (0.45-0.65) yine elenir; pg_partman vb. `_EXCLUDED_SCHEMAS` ile zaten dışlı (eşikten bağımsız). **Buton/background eklenmedi** (kullanıcı direktifi: süreç-içi, gereksiz buton yok). Test: FK-loop+synthetic suite 60 passed (xfail hariç) + ruff 0. Backend restart; migration YOK. ÖNCEKİ v3.77.2 (canlı-log teşhisi fix paketi: **FK çözüm iyileştirme** [head-noun çok-kelime `PrimaryOrganizationUnitId`→`organizationunit` + rol-önekli USER self-ref `CreateUserId`/`ParsUserId`→kendi USER tablosu (head!=root gate) + PK-farkında hedef seçimi: PK'sız VIEW (`PROBLEMID`→`NFN_VW_*`) atla, PK'lı tabloyu tercih et], **golden_sql admin company_id** [None→source firmasından çöz; admin başarılı text-to-sql `company_id NOT NULL` crash fix → öğrenilen sorgu kaybı bitti], **geniş-tablo SELECT* örnekleme son-çare fallback** [812-kolon `T_ICT_DYNAMIC_DATA` explicit-kolon listesi patlayınca SELECT* ile örneklenir], **FK uyarı seli→kaynak başına tek özet INFO** [detay `ds_fk_diagnostics` Tanılama ekranında; v3.60.0 per-tablo WARNING seli (1523×) çift-yazımdı] + `GUID`/`PARENTGUID` `NON_FK_COLUMN_NAMES` skip (754 sahte uyarı), **ml_training mesaj netliği** [`chunk bulunamadı`/`eğitim üretilemedi` → `rag_chunks` bilgi-tabanı eksik; DB 'Örnek Veri Topla' ile karışmasın]. Code-review [3 finder+verify → çift `_resolve_target_pk` çağrısı fix, probe sonucu reuse] + gstack-review adversarial [SHIP-ready; `_safe_identifier` denylist P2 defense-in-depth → REFACTOR_BACKLOG item-10]. Test: ruff 0 + FK 49/49 + standalone helper. Backend restart gerek; migration YOK. ÖNCEKİ v3.77.1 (CEO-review Tema-1 residual 1-3: R1 rapor fallback PROMINENT uyari, R2 wizard outcome kaydi [learning_recorder], R3 enrichment kapsama rozeti [columns_enriched persist+mig056+UI]. Code-review 2 fix [R3 abort-rollback, R2 admin company_id]. mig056 apply+restart. ONCEKI v3.77.0 (CEO-review Tema-1 Dilim-1: **FK kapali-dongu** — `ds_fk_diagnostics` (cozulemeyen FK kok-neden, persist+trend) + GET `/fk-diagnostics`; FK observability tabina verify/reject + bulk-verify butonlari (mevcut backend'i bagla) + 'Cozulemeyen FK'lar' tanilama bolumu. Backend restart + mig055 apply. ONCEKI v3.76.5 (Hata Izleme log .xlsx export REDESIGN: yeni admin-only `POST /api/system/errors/export` — `ids` verilirse o kayitlar, yoksa FILTREYE UYAN TUM kayitlar [DB'den, liste 100-cap'i degil, 50k tavan] + TAM detay/full-traceback; `_build_errors_where` list+export filtresini senkron tutar. 'Tumunu sec'=filtreli-tum. Backend restart gerekir. ONCEKI v3.76.4 (design-review polish: Hata Izleme touch-target [checkbox kolonu 28px/kutu 16px] + 'Tumunu sec' liste ustune sol-hiza [checkbox kolonuna demir atar]; izole-render kanitli. ATLANDI: accent-token [renk kaymasi riski], _tooltip.css global [tooltip'leri bozma riski] — hedefli fix korundu. ONCEKI v3.76.3 (design-review tasarim-tutarlilik: Hata Izleme XLSX UI checkbox amber->var(--blue) [secim konvansiyonu], export ikonu #1d8348->#4caf50, yeni kontrollere focus-visible [a11y]; izole-render kanitli. ONCEKI v3.76.2 (frontend: **arama-kutusu X (temizle) konumu KÖK fix** [`_tooltip.css [data-tooltip]{position:relative}` bundle'da sonra+eşit specificity ile `.ds-search-clear{position:absolute}`'u eziyordu → X tam-genişlik input'un ALTINA/sola kayıyordu; şema+yetki+perm aramalarında AYNI; `.ds-search-clear[data-tooltip]{position:absolute}` (0,2,0>0,1,0) ile çözüldü, **headless-chrome render + computed-style kanıtlı**; v3.73.0 `left`'i denemişti, asıl sorun `position`'dı] + **Hata İzleme seçili-log .xlsx indir** [satır checkbox + tümünü-seç, mevcut `/api/db/export/excel` reuse, yeni backend yok]. ÖNCEKİ v3.76.1 (code-review [gstack /review] 3 P2 fix: **(1) self-ref guard** — `_resolve_target_pk` self-ref'te FK kolonunun KENDİSİNİ hedef PK seçemez [root-türevli `parent_id` geri gelip `parent_id→parent_id` çöp self-FK üretiyordu, PK-siz hiyerarşi tablosunda]; **(2) fuzzy coverage fan-out-immune** — `build_sample_validate_sql` 4 dialect `COUNT(*)/COUNT(t.tc)` → `COUNT(DISTINCT s.v)/COUNT(DISTINCT CASE WHEN matched)` => non-unique hedef anahtarda coverage fan-out'la şişmez [fuzzy'nin TEK false-positive kapısı gerçekleşti] + ratio `min(.,1.0)` clamp; **(3) .py EOL kalıcı** — `.gitattributes '*.py text eol=lf'` + config.py LF renormalize [Windows bump-script CRLF flip = 273-satır sahte diff/bozuk blame]. INFORMATIONAL → RB-v3.76.1. Test: self-ref regression + 49/49 pytest. ÖNCEKİ v3.76.0: FK inference **G4a — sample-validation'lı FUZZY hedef eşleşme** [OPT-IN, default OFF]. İsim-tier'ları [exact/plural/entity-token/prefix/head/self] boş dönen FK kolonları için root/head ile **herhangi bir token'ı paylaşan** tablolar aday yapılır; **yalnız sample coverage ≥ %50** ile persist → false-positive yok, genel/her-dialect, hardcoded-abbreviation yok. Amaç: kötü-adlandırılmış şemada naming'in kaçırdığı FK'ları gerçek-veriyle doğrulayarak yakala → daha çok JOIN → daha doğru Text-to-SQL. Oracle CSN gap-analizi: kod yapıyı çözüyor [UPPER/composite-PK/_ID], gerçek gap teşhis SQL'iyle ölçülecek. Code-review [2 ajan] **5 fix**: opt-in `payload.fuzzy` [auto-coupling kaldırıldı], Oracle `call_timeout=3s` [8×N probe asmasın], `fuzzy_resolved` gerçek-persist + `fuzzy_attempted` observability, self-exclusion schema+ad, `_resolve_target_pk` extraction. +6 pytest. Altitude: RB-v3.76.0. ÖNCEKİ v3.75.0: Geniş-tablo TAM öğrenme + FK gürültü temizliği + enrichment güvenilirliği — kullanıcı bulgusu: ONEDESKPG'de 312-kolonlu tablo "LLM BEKLEYEN"/etiketsiz + FK uyarı seli [GCRecId 93×]. **(1) Kolon cap KALDIRILDI** ["max 50 yok, ne kadar varsa öğren"]: db_smart listing 50→hepsi, örnekleme 50→500, enrich 500→2000, text_to_sql cols[:80]→tüm + göster 50→150, rapor şema 80→500. **(2) FK gürültü:** gcrecid/recid denylist [framework satır-kimliği, 93 sahte uyarı sustu], code/key/no suffix çıktı [154 sahte], ParentId self-ref. **(3) Enrichment:** chunk 80→40 [max_tokens=4096 truncation = "312 BEKLEYEN" kök nedeni], retry double-wrap fix, kolon-toptan-başarısızlık Hata İzleme'ye yazılır. Korunan: `_fill_missing_columns`/`_check_column_hallucination`/relevance-pruning. Code-review medium 5 fix. Test: FK 66/66 + enrichment 46/46. ÖNCEKİ v3.74.2: Kolon enrichment VERİ KAYBI fix — ana LLM çağrısı çok-kolonlu tabloda bazı kolonları düşürüyordu ["—" etiketsiz]; `_fill_missing_columns` ile chunk'lı doldurma [case-insensitive + retry]. ÖNCEKİ v3.74.1: BUG1 FK-loop empty-state canlı FK sayısından + BUG2 Akıllı Keşif rapor timeout sınıflandırma/yükseltme. ÖNCEKİ v3.74.0: Identity Cascade — keşifte declared PK yoksa UNIQUE INDEX'ten PK yakala [4 dialect: PG/Oracle/MSSQL/MySQL, try/except izole]. KÖK: MSSQL→PG migre DB'de PK/FK constraint düşmüş ama `PK_<tablo>` unique-index duruyor → keşif yalnız PRIMARY KEY sorguluyordu → is_pk boş → FK inference target_pk_not_found. Cascade: declared > unique-index > isim-sezgisi + provenance. ÖNCEKİ v3.73.0: FK kolon-kökü prefix-soyma + esnek PK + UI arama-X hiza. Detaylı geçmiş: `app/core/config.py` `APP_VERSION` + `git log` + `.agents/plans/`.)

**Geçmiş versiyon notları:**

3.42.1 (İki frontend duplicate fix [v3.42.0 üstüne]: **(1) deep_think DB-Only çift yanıt** — v3.42.0 websocket import fix'i (`manager`→`ws_manager`) `db_query_complete` push'unu aktifleştirdi; kullanıcı sohbet ekranındayken SSE ZATEN render etmişken `addMessageFromWS` 2. kez ekliyordu — scope-denial yanıtında SSE bubble kaydedilmiş message_id taşımadığından `[data-message-id]` dedup'ı kaçırıyor (çift Yetki Notu), başarılı sorguda `content[:200]` narrative'ini TABLO yerine gösteriyordu. Fix (`websocket_client.js handleDbQueryComplete`): `!statusEl` guard — `dbqs_jobId` placeholder kartı DOM'daysa (kullanıcı ekranda) SSE'ye bırak; ekran dışındaysa (geri dönünce görsün) websocket render eder. Cross-screen bildirim (placeholder güncelleme + notification) korunur. **(2) Akıllı Keşif WHERE 2×** — AST editör `_flushPatch` `base_ast: state.ast` (optimistic-apply SONRASI, yeni filtreyi İÇEREN) gönderiyordu; backend session-ast boşken (`db_smart_api.py:474` base_ast fallback) `add_filter`'ı TEKRAR uyguluyordu → WHERE çift (chip + önizleme/assemble SQL). PRE-EXISTING (v3.38.4 `base_ast` eklendi) ama v3.42.0 WHERE/ORDER BY işi önizlemeye filtre koyunca GÖRÜNÜR oldu; execution doğruydu (LLM tekrarı dedup'lar). Fix: `base_ast: rollbackAst` (`pending[0].prevAst`, optimistic-ÖNCESİ snapshot) → backend op'u temiz taban üzerine TEK kez uygular; 409-guard korunur. Read-first Graphify (`_scope_restricted_message` lookup) + commit→mine akışı kullanıldı.)
3.42.0 (Akıllı Keşif WHERE/ORDER BY uçtan uca + v3.42 batch: **Wizard önizleme/kaydet SQL'i** artık WHERE (AST editör chip) + ORDER BY (SIRALAMA chip) yansıtır — `_buildWizardState` filters/order_by'ı assemble payload'ına ekler, backend `query_assembler`/`ast_renderer` zaten tüketir (`{expr,op,value}`/`{expr,dir}`); kaydet persist + yeniden-aç rehydrate (`_orderByToRenderer`/`_orderByToChip` köprü). **İcra** (`/generate-report` LLM): filters/order_by `_build_prompt`'a **ZORUNLU kısıt** → üretilen+çalıştırılan SQL WHERE/ORDER BY içerir. Dış-kaynak icrada `assemble` KULLANILAMAZ — `inject_rls` non-admin'e `company_id` enjekte eder, Oracle tablosunda o kolon yok → LLM+SafeSQLExecutor (tenant izolasyonu source-level + whitelist). **Bind-inline display** (`query_assembler.inline_binds`): deterministik önizleme SQL'inde `%(v_1)s`→gerçek değer (Oracle `:name`/MSSQL `@name` prefix-overlap güvenli, injection-safe `_literal` quote-escape). **SQL modal:** boş-talepte yalnız deterministik üst (nihai LLM bölümü gizli). **Export:** Rapor Sonucu Excel/Word/PDF (`/api/db/export/{fmt}` reuse, pozisyonel→dict) + Üretilen SQL kopyala. **deep_think Faz1:** follow-up'ta golden/cache short-circuit skip (`_is_followup` — "bu siparişin müşteri detayı" artık tüm müşteri listesi getirmez) + `join_planner` (FK grafiğinden deterministik join, bridge-leak guard). **Infra:** ORA-12170 `_is_infra_db_error` fast-fail, `safe_sql_executor` ORA/DPY/TNS kodunu korur (Fortify-safe, host/port'suz), websocket `ws_manager` import fix, `EmbeddingManager` SINGLETON (`__new__` + RLock — per-call cold-load hang fix). code-review 2 tur 10+ bulgu (inline_binds Oracle overlap, prompt `_literal`, finalSql cache key, fallback uyarısı, bridge-leak, init-race, narrative dead-flag, dup/dead temizlik) hepsi fix + smoke.)
3.41.6 (Veritabanında Ara & Akıllı Keşif tutarlılık paketi: wizard generate_report whitelist'i picker yerine TAM `can_execute` scope'tan kurulur → not'tan gelen yetkili-ama-seçilmemiş tablo (ör. ADRESLER) yanlış reddedilmez; red mesajları reddedilen tabloyu `can_view`-gated adlandırır (site-1 403 picker + site-2 200 üretilen-SQL); deep_think "Yetki Notu" yalnız GERÇEK scope reddinde gösterilir (parse/LLM hatası artık yetkili tabloyu yanlış reddetmez — `_is_scope_denial_error`); DIAGNOSTIC/yorum-only LLM çıktısı (text_to_sql + llm_generate_report) generic "Yalnızca SELECT" / sessiz `SELECT *` fallback yerine açıklamayı yüzeye çıkarır (geçerli SQL varsa override yok — code-review fix); SQL önizleme modalı geniş+sabit 85vh+iç scroll ("Kapat" daima görünür); kayıtlı rapor **Sil** → `close()` `_opts` sıfırlamadan ÖNCE `onDeleted` yakalanır → kart anında kaybolur, F5 gereksiz. NOT: 3.38.0–3.41.5 ara notları için git log / `config.py APP_VERSION` yorumuna bakın — README versiyon başlığı bu sürümde 3.37.8'den senkronlandı.)
