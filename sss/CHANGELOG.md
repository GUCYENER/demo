...
### ✨ Yeni Eklenenler
- **ML Answer Formatlayıcı:** Düz metin halindeki ML cevapları ayrıştırılarak renkli ve düzenli HTML formatında listelenecek hale getirildi. "[ARANABİLİR]" tagları için rozetler (badge) eklendi.

### 🛠️ Düzeltilen
- **LLM Text-to-SQL Tablo Seçimi:** `text_to_sql.py` içindeki sistem promptunda sadece ana tek tabloya zorlanma kısıtlaması esnetilerek LLM'in gerektiğinde tabloları alan bazlı (id) JOIN yapabilmesinin önü açıldı.
...
## [3.6.3] - 11.04.2026
### Added
- Text-to-SQL dinamik dialect destegi saglandi (PostgreSQL, MSSQL, MySQL, Oracle zel tirnak kurallari eklendi).
- DS QA Generator yerine sadece kati tablo mimarisini (schema_record) kaydeden DSSchemaLearner (v4.0.0) aktif edildi.
- DB arama sonulari iin RAG benzeri dinamik stream metin ve Markdown tablo iktisi eklendi.

### Fixed
- Text-to-SQL "relation does not exist" hatasi PostgreSQL iin ift tirnak (") force edilerek zld.
- LLM'nin var olmayan kolonlari uydurmasi "sadece var olan stunlara bak" kurali ile engellendi.
